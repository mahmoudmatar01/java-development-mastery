...
This section demonstrates the Software Development Life Cycle (SDLC) for managing and ensuring the success of a new software project in the software business. SDLC has 6 phases :
...
Java seduced programmers with Its friendly syntax, object-oriented features, memory management, and best of all all-the promise of portability. The lure of write-once/run anywhere Is just too strong.
...
Any device capable of running Java will be able to Interpret and translate this file into something It can run. The compiled bytecode is platform-independent.
### Virtual Machhines :
...
You'll type a source code file, compile It using the Java compiler, and then run the compiled bytecode on a . Java virtual machine.
...
powerful. It comes in three flavors: Micro Edition (J2ME),
Standard Edition (J2SE) and Enterprise Edition (J2EE).
Becomes the language of Chola for new enterprises (especially web-based) and mobile applications.
...
## Q..: I see Java 2 and Java 5.0, but was there Java 3-.d 4? And why Is It Java 5.0 but not Java 2.0?
...
will explain how the Dog should bark. Your methods must be declared inside a class (in other words, within the curly braces of the class).
```java 
public class Dog{
...
for how that method should be performed. Method code is basically a set of statements, and for now, you can think of a method
...
```
Next, the JWM runs everything between the curly braces { }of your main
method. Every Java application has to have at least one class. and at least one main method (not one main per class; just one main per application)
```java
...
class means: this is a java keyword to create a class 
...
main means: the name of the method 
...
Java is an object-oriented(OO) language. It's not Like the old days when you had steam-driven compliers and wrote one monolithic source file with a file of procedures. 
...
used variables In two places object state (instance variables), and local variables (variables declared within a method). Later, we'll use variables as arguments (values sent to a method by the calling code), and as return types (values sent back to the caller of the method). You've seen variables declared as simple primitive integer values (type int).
- When you think of Java variables, think of cups. Coffee cups, tea cups, giant cups that hold d lots and lots of beer, those big cups the popcorn comes in at the movies, cups with curvy, sexy handles, and cups with metallic trim that you learned can never, ever go in the microwave.
- A variable is just a cup. A container. It holds something.


### Primitive Types
- Type BitDepth Value Range boolean and char, boolean (JVM ..podfic) true or false, and char 16 bits 0 to 65535
...
powerOn = IsFun;
long big = 3456789j
float f = 32.5f;  // notice 'f' gotta have that with a float, because Java thinks anything with a floating point is a double unless you use 'f'
```
...
- Based on what you know about the size and type of the primitive variables, see if you can figure out which of these are legal and which aren't, We haven't covered all the rules yet, so on some of these you'll have to use your best judgment. Tip: The compiler always errors on the side of safety.
```java
...
- It must start with a letter, underscore U, or dollar sign ($). You can't start a name with a number.
- After the first character, you can u.. numbers as well. Just don't start It with a number.
...
- Linke The Object And The Reference :
    ...
    Dog myDog = new Dog () ;
    ```    
    Assign the new Dog to the reference variable myDog.ln other words programs the remote control.
...
Book c=new Book();
```
- Declare two book reference variables. Create two new Objects to the reference. Assign the Book object to the reference variable. The Two Book objects are now living on the heap.
references:2 
Objects:2
...
c = b;
```
- Assign the value of variable b to variable c. By now you know what it means. The bits inside the variable are copied, and that new copy Is stuffed into variable c. Both bands c refer to the same object. References:3
Objects: 2
...
c = null ;
```
- Assign the value nu 11 to variable c. This makes c a null reference, meaning It doesn't refer to anything. But It's still a reference variable, and another Book object can still be assigned to It. Object 2 still has an active reference (b), and as long as It does, the object Is not eligible for GC.
Active References: 1
null References: 1
Reachable Objects: 1
Abandoned Objects: 1
...
String name = "Donny"
```
But when you don't initialize an instance variable, what happens when you call a getter method? In other words, what is the value of an instance variable before initializing it?
```java
class PoorDog{
...
• *The Dot Operator*: One of the primary uses of an object reference variable is to access the members of the class for this object, an instance of its class. That is, an object reference variable is useful for accessing the methods and instance variables associated with an object. This access is performed with the dot (“.”) operator. We call a method associated with an object by using the reference variable name, followed by the dot operator and then the method name and its parameters.
```java
...
The first set of modifiers we discuss is known as access control modifiers, as they control the level of access (also known as visibility) that the defining class grants to other classes in the context of a larger Java program. The ability to limit access among classes supports a key principle of object orientation known as encapsulation (see later). In general, the different access control modifiers and their meaning are as follows:
...
private int count;
```
- where private is the modifier, it is the type, and the count is the identifier. Because we did not declare an initial value, it automatically receives the default of zero as a base-type integer.
...
[modifiers] returnType methodName(type1 param1 , ..., type params) {
// method body . . .
}
- The method name can be any valid Java identifier. The list of parameters and their types declares the local variables that correspond to the values that are to be passed as arguments to this method.
- Each type declaration type can be any Java type name and each parami can be any distinct Java identifier. This list of parameters and their types can be empty, which signifies that there are no values to be passed to this method when it is invoked.
...
- However, if a class defines one or more non-default constructors, no default constructor will be provided.
...
#### Concatenation 
The primary operation for combining strings is called concatenation, which takes a string P and a string Q and combines them into a new string, denoted P+ Q, which consists of all the characters of P followed by all the characters of Q. In Java, the “+” operation performs concatenation when acting on two strings, as follows:
```java
String term = "over" + "load";
```
This statement defines a variable named term that references a string with the value "overload".
...
### Wrapper Types :
Java defines a wrapper class for each base type. An instance of each wrapper type stores a single value of the corresponding base type. In the next Table, we show the base types and their corresponding wrapper class, along with examples of how objects are created and accessed.
...
- Each class is given its corresponding base type and example expressions for creating and accessing such objects.
...
- In any context for which an Integer is expected (for example, as a parameter), an int value k can be expressed, in which case Java automatically boxes the int, with an implicit call to a new Integer(k). In reverse, in any context in which an
...
- It is a dangerous mistake to attempt to index into an array using a number outside the range from 0 to a.length−1.
...
int[ ] primes;
```
- Because arrays are a reference type, this declares the variable primes to be a reference to an array of integer values, but it does not immediately construct any such array. There are two ways to create an array.
...
#### Logical Operators
Java supports the standard comparison operators between numbers:
```
< less than
...
> greater than
```
- The type of the result of any of these comparisons is a boolean.
...
if (booleanExpression)
    ...
    falseBody
```
- where boolean expression is a boolean expression and trueBody and falseBody are each either a single statement or a block of statements enclosed in braces (“{” and “}”). Note that, unlike some similar languages, the value tested by an if statement in Java must be a boolean expression. In particular, it is definitely not an integer expression.
...
```
The switch statement evaluates an integer, string, or enum expression and causes the control flow to jump to the code location labeled with the value of this expression. If there is no matching label, then the control flow jumps to the location labeled “default.” This is the only explicit jump performed by the switch statement, however, so the flow of control “falls through” to the next case if the code for a case is not ended with a break statement (which causes control flow to jump to the end).
...
- As an example, here is a loop that advances an index through an array named data until finding an entry with a value target or reaching the end of the array.
```java
int j = 0;
...
#### Do-While Loops
Java has another form of the while loop that allows the boolean condition to be checked at the end of each pass of the loop rather than before each pass. This form is known as a do-while loop, and has the syntax shown below:
```
do
    loopBody
while (booleanExpression)
...
- The boolean condition will be evaluated immediately before each potential iteration of the loop. It should be expressed similarly to a while-loop condition, in that if it is true, the loop body is executed, and if false, the loop is exited and the program continues to the next statement beyond the for-loop body.
...
the display and input of text and numbers. Java also provides methods for dealing with graphical objects, images, sounds, Web pages, and mouse events (such as clicks, mouseovers, and dragging). Moreover, many of these input and output methods can be used in either stand-alone programs or in applets.
...
- You use the dot operator (.) on a reference variable 10 say, "use the thing before the dot to get me the thing after the dot." For example
```java
myDog.bark() ;
```
...
### How can I access private attributes outside a class
...
- There are Shapes, and they all rotate and plavSound. So I abstracted out the common features and put them into a new class called Shape.
```plantuml
@startuml
class Shape {
  + rotate() : void
...
- The.The shape class is called the Superclass of the other four classes. The other four are the subclasses of Shape. The Subclasses Inherit the methods of the superclass. In other words. If the Shape class has the functionality, then the subclasses automatically get that same functionality.
...
- Tub and Bathroom are related, but not through inheritance. The tub and Bathroom are joined by a HAS-A relationship. Does it make sense to say "Bathroom HAS-A TUb"? If yes, then it means that Bathroom has a Tub instance variable. In other words, Bathroom has a reference to a Tub, but Bathroom does not extend Tub and vice-versa.
...
Keep in mind that the inheritance IS-A relationship works in only one directional!
- Triangle IS-AShape makes sense, so you can have Triangle extend Shape. But the reverse-shaped IS-A Triangle-does not make sense, so Shape should not extend Triangle. Remember that the IS-A relationship implies that if X IS-A y. then X can do anything a Y can do (and possibly more).
...
When you override a method from a superclass, you're agreeing to fulfill the contract. The contract that says. for example, I take no
